...
const CyberSecurityNews = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gray-800 text-white mb-7">
      ...
      <div className="mx-auto my-auto max-w-6xl overflow-hidden rounded-xl bg-white p-6 text-black shadow-md">
        ...
        {articles.map((article, index) => (
          <NewsCard key={index} urlToImage={(article as any).urlToImage} title={(article as any).title} url={(article as any).url} description={(article as any).description} />
        ))}
        <button
          onClick={() => router.reload()}
          className="mt-4 rounded bg-blue-800 px-4 py-2 text-lg font-bold text-white hover:bg-blue-700 transition duration-300"
        >
